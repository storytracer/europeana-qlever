...
- ``group`` and ``map`` exports are built by DuckDB composition from
  raw Parquets.
"""
...
_SET_DESCRIPTIONS: dict[str, str] = {
    "pipeline": "All final exports: raw + group + maps",
...
class ExportRegistry:
    ...
    def _build(self) -> dict[str, Export]:
        ...
        # 4. group_items / map_* — declared depends_on.
        for info in export_classes().values():
            if info.export_type in ("group", "map"):
                exports[info.table_name] = CompositeExport(
                    name=info.table_name,
...
class ExportPipeline:
    ...
    def _run_composite(self, export: CompositeExport, parquet_path: Path) -> tuple[int, float]:
        ...
        # group_items uses a shared-then-per-chunk path with CREATE OR
        # REPLACE TEMP TABLE; its scalar-only final SELECT is cheap.
    ...
    def _run_composite_chunked(
        ...
        """Chunked composition for group_items.
