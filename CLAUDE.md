...
uv run europeana-qlever -d WORK_DIR analyze static --query-set enriched  # Analyze enriched queries
...
- **Query generation** (`query.py`) uses `QueryBuilder` to dynamically generate SPARQL queries from composable fragments. `QueryFilters` dataclass carries filter parameters (country, type, rights category, year range, etc.). `QuerySpec` dataclass wraps each query with metadata: `sparql` (for simple exports), `compose_steps` (for composite DuckDB exports), `depends_on` (dependency list), and `description`. Queries are grouped into base (6), component (8), enriched (1), and example (11) categories — 18 user-facing queries plus 8 component building blocks. The builder's registry methods return `dict[str, QuerySpec]` consumed by `export_all()`.
...
- **Composition SQL** (`compose.py`) generates DuckDB SQL templates for composite exports as a list of `ComposeStep` objects (12 steps for `items_enriched`). Templates use `{exports_dir}` as a placeholder replaced at execution time. `items_enriched_steps()` composes the flagship enriched export from 8 component tables + agents, with language resolution (en → vernacular → extras → any), multi-valued property aggregation using native Parquet types (`LIST<VARCHAR>` for subjects/dates/years/languages, `LIST<STRUCT<name VARCHAR, uri VARCHAR>>` for creators), and creator label resolution via agent entity join. The final step is marked `is_final=True` for COPY-to-Parquet wrapping.
...
`QuerySpec` wraps each export with: `name`, `sparql` (for simple SPARQL exports), `compose_steps` (list of `ComposeStep` for DuckDB composite exports, where each step has `name`, `sql`, and `is_final`), `depends_on` (list of dependency names), and `description`. The `is_composite` property distinguishes the two types. Registry methods (`all_base_queries()`, `all_enriched_queries()`, etc.) return `dict[str, QuerySpec]`.
...
**Language resolution** uses a parallel English + vernacular model. Component tables export raw values with language tags, and the `items_enriched` composition SQL (DuckDB) applies language resolution: en → vernacular → extras → any, producing parallel columns (`title_en`, `title_native`, `title_native_lang`, `title`). Entity labels (skos:prefLabel) use a simpler chain: en → user extras → any. Users add languages via `QueryBuilder(languages=[...])` or the `--language` CLI flag.
